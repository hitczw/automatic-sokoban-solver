...
class maze_solver {
private:
	bool bfs_maze_solver(const vector<vector<char>> &maze_matrix, const point &end, const point& person_point) {
		//广度优先法解决迷宫,返回迷宫是否有解
		//仅当终点为通道时候有解
	...
	bool dfs_maze_solver(const vector<vector<char>> &maze_matrix, const point &end, const point& person_point) {
		//深度优先,回溯法
		if (maze_matrix[end.x][end.y] != 1) {//如果终点不是通道
			return false;
		}
	...
	bool Astar(const vector<vector<char>> &maze_matrix, const point &end, const point& person_point) {
		//A*算法解迷宫
		//此算法对大地图效率体现有效
		//小地图不明显
		if (maze_matrix[end.x][end.y] != 1) {//如果终点不是通道
...
public:
	...
	bool operator()(METHOD method, const vector<vector<char>> &maze_matrix, const point &end, const point& person_point) {
		switch (method) {
		case bfs:
	...
	vector<point> get_pace(const vector<vector<char>> &maze_matrix, const point &end, const point& start) {
		//函数换位置,不能写在这里
		if (start == end) {
			vector<point> result;
